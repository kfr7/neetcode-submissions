class Solution:
    def generateParenthesis(self, n: int) -> List[str]:
        ...
        def helper(leftRem, rightRem, build, solutions):
            if leftRem == 0 and rightRem == 0:  # base case
                return solutions.append(build)
            elif (leftRem == rightRem): # must do
                helper(leftRem-1, rightRem, build + '(', solutions)
            elif (leftRem == 0): # must do
                helper(leftRem, rightRem-1, build + ')', solutions)
            else:   # either one is fine
                helper(leftRem-1, rightRem, build + '(', solutions)
                helper(leftRem, rightRem-1, build + ')', solutions)
        
        listSolutions = []
        helper(n, n, '', listSolutions)
