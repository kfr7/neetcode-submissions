    ...
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        for i in range(len(heights)):
            ...
            if len(heightStack) == 0 or heightStack[-1] < heights[i]:
                ...
                # that are bigger or equal to it and replace the last index of the one you popped
                # tempLowestIndex = None
                # while len(heightStack) > 0 and heightStack[-1] >= heights[i]:
                #     tempLowestIndex = indexStack.pop()
                #     heightStack.pop()   # can disregard since only our minimum matters
                # largestRectangle = max(largestRectangle, heights[i] * (i - tempLowestIndex))
